function [EuroPutClosed AmerPutMS AmerPut EEP theta y] = MSPrice(S,Strike,T,r,q,params,trap,method,A,B,N,NumTerms,yinf,hi)
...
%    hi = upper bound for bounded optimization
...
lo = max(2,theta);
y = fminbnd(@(p) -MSPutHeston(p,theta,Strike,params,r,q,T,NumTerms),lo,hi);
if y<theta
    y = theta;
end
...
% MaxIter = 10000;
% start = max(2,theta);
% y = fmincon(@(p) -MSPutHeston(p,theta,Strike,params,r,q,T,NumTerms),start,[],[],[],[],theta,[]);
% y = GoldenSearch(lo,hi,tol,MaxIter,theta,Strike,params,r,q,T,NumTerms);
