...
namespace Mikhailov_and_Nogel_Estimation_DJIA
{
    class NelderMeadAlgo
    {
        ...
        public delegate double ObjFun(double[] x,double tau,double[] tau0,double[,] param0,MktData data,double S,double[] K,double r,double q,int trap,
                                      double[] X,double[] W,int LossFunction,double[] lb, double[] ub);

        // Nelder Mead Algorithm ===============================================================================================
        public double[] NelderMead(ObjFun f,int N,int MaxIters,double Tolerance,double[,] x,double tau,double[] tau0,double[,] param0,
                                   MktData data,double S,double[] K,double r,double q,int trap,double[] X,double[] W,int LossFunction,double[] lb,double[] ub)
        {
            int i,j;
            int NumIters = 0;
            ...
            step0:
            NumIters = NumIters + 1;
            Console.WriteLine("Nelder Mead iteration {0}", NumIters);
            for(j=0;j<=N;j++)
            {
                ...
                {
                    z[i] = x[i,j];
                    F[j][0] = f(z,tau,tau0,param0,data,S,K,r,q,trap,X,W,LossFunction,lb,ub);    // Function values
            ...
            double f1 = f(x1,tau,tau0,param0,data,S,K,r,q,trap,X,W,LossFunction,lb,ub);

            // Last best vector y(N) and function value fn
            double[] xn = new Double[N]; for(i=0;i<=N-1;i++) xn[i] = y[i,N-1];
            double fn = f(xn,tau,tau0,param0,data,S,K,r,q,trap,X,W,LossFunction,lb,ub);

            // Worst vector y(N+1) and function value fn1
            double[] xn1 = new Double[N]; for(i=0;i<=N-1;i++) xn1[i] = y[i,N];
            double fn1 = f(xn1,tau,tau0,param0,data,S,K,r,q,trap,X,W,LossFunction,lb,ub);
            ...
            double fm = f(xm,tau,tau0,param0,data,S,K,r,q,trap,X,W,LossFunction,lb,ub);

            // Reflection point xr and function fr
            double[] xr = new Double[N]; xr = VSub(VAdd(xm,xm),xn1);
            double fr = f(xr,tau,tau0,param0,data,S,K,r,q,trap,X,W,LossFunction,lb,ub);

            // Expansion point xe and function fe
            double[] xe = new Double[N]; xe = VSub(VAdd(xr,xr),xm);
            double fe = f(xe,tau,tau0,param0,data,S,K,r,q,trap,X,W,LossFunction,lb,ub);

            // Outside contraction point and function foc
            double[] xoc = new Double[N]; xoc = VAdd(VMult(xr,0.5),VMult(xm,0.5));
            double foc = f(xoc,tau,tau0,param0,data,S,K,r,q,trap,X,W,LossFunction,lb,ub);

            // Inside contraction point and function foc
            double[] xic = new Double[N]; xic = VAdd(VMult(xm,0.5),VMult(xn1,0.5));
            double fic = f(xic,tau,tau0,param0,data,S,K,r,q,trap,X,W,LossFunction,lb,ub);
        ...
        public double[] VMean(double[,] X,int N)
        {
