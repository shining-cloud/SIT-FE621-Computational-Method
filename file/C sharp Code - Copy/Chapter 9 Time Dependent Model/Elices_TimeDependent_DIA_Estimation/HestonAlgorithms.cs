...
namespace Elices_TimeDependent_DIA_Estimation
{
    ...
    {
        // Heston Integrand
        public double HestonProb(double phi,HParam param,OPSet opsettings,double K,double T,int Pnum)
        {
            double S = opsettings.S;
            double r = opsettings.r;
            double q = opsettings.q;
            int trap = opsettings.trap;
        ...
        public double HestonPriceGaussLaguerre(HParam param,OPSet opsettings,double K,double T,string PutCall,double[] x,double[] w)
        {
            ...
            for(int j=0;j<=31;j++)
            {
                int1[j] = w[j] * HestonProb(x[j],param,opsettings,K,T,1);
                int2[j] = w[j] * HestonProb(x[j],param,opsettings,K,T,2);
            }
