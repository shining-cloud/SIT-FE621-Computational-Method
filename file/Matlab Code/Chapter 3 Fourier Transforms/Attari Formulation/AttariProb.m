function y = AttariProb(phi,kappa,theta,lambda,rho,sigma,tau,K,S,r,q,v,Trap)
...
f = exp(C + D*v - i*phi*r*tau);

% The "L" function
L = log(exp(-r*tau)*K/S);

% The Attari (2004) integrand
y = ((real(f) + imag(f)/phi)*cos(L*phi) ...
   + (imag(f) - real(f)/phi)*sin(L*phi)) / (1+phi^2);
