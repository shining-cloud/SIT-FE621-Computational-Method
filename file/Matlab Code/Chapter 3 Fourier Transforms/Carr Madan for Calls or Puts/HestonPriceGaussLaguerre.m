function y = HestonPriceGaussLaguerre(Integrand,alpha,S,K,T,r,q,kappa,theta,sigma,lambda,v0,rho,trap,x,w,PutCall)
...
if strcmp(Integrand,'Heston')    % Heston formulation of the integrand	
	...
	if strcmp(PutCall,'C')
		...
		y = Call - S*exp(-q*T) + K*exp(-r*T);
	end
elseif strcmp(Integrand,'CarrMadan')   % Carr-Madan formulation of the integrand
	for k=1:length(x);
		int1(k) = w(k)*CarrMadanIntegrand(x(k),alpha,kappa,theta,lambda,rho,sigma,T,K,S,r,v0,trap,PutCall);
	end
	% The Option Price
	if strcmp(PutCall,'C')
