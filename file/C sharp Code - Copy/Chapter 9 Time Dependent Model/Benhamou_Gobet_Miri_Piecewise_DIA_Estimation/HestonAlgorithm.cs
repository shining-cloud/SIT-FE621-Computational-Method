...
namespace Benhamou_Gobet_Miri_Piecewise_DIA_Estimation
{
    ...
    {
        // Heston Integrand
        public double HestonProb(double phi,HParam param,OPSet opset,double K,double T,int Pnum)
        {
        ...
        public double HestonPriceGaussLaguerre(HParam param,OPSet opset,double K,double T,double[] x,double[] w)
        {
            ...
            for(int j=0;j<=31;j++)
            {
                int1[j] = w[j] * HestonProb(x[j],param,opset,K,T,1);
                int2[j] = w[j] * HestonProb(x[j],param,opset,K,T,2);
            }
